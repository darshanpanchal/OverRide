...
class AddChildViewController: UIViewController {
    ...
    var addChildParameters:[String:Any] = [:]
    ...
    func setUp(){
        ...
        self.addChildParameters["country_code"] = "1"
        ...
        self.imageViewCamera.layer.borderWidth = 2.5
    ...
    func configureFormDatePicker(){
        ...
        let doneButton = UIBarButtonItem(title: Vocabulary.getWordFromKey(key:"Done"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(AddChildViewController.doneFormDatePicker))
        ...
        title.sizeToFit()
        let cancelButton = UIBarButtonItem(title:Vocabulary.getWordFromKey(key:"Cancel"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(AddChildViewController.cancelFormDatePicker))
    ...
    @objc func doneFormDatePicker(){
        ...
        self.addChildParameters["dob"] = "\(self.fromDatePicker.date.yyyyMMdd)"
    ...
    func configurePasswordTextField(){
        ...
        objPassword =  UIButton.init(type: .system)// UIButton.init(frame: CGRect.init(x: 0, y: 0, width: 30, height: 30))
        objPassword!.frame = CGRect.init(x: 0, y: 0, width: 30, height: 30)
        objPassword!.setImage(UIImage.init(named: "ic_eye_off")?.withRenderingMode(.alwaysTemplate), for: .normal)
        objPassword!.tintColor = UIColor.init(hexString: "#707070")
    ...
    func configureConfirmPasswordTextField(){
        ...
        objConfirmPassword = UIButton.init(type: .system)///UIButton.init(frame: CGRect.init(x: 0, y: 0, width: 30, height: 30))
        objConfirmPassword!.frame = CGRect.init(x: 0, y: 0, width: 30, height: 30)
        ...
        objConfirmPassword!.tintColor = UIColor.init(hexString: "#707070")
    ...
    func isValidSignUp()->Bool{
        ...
        guard "\(self.addChildParameters["name"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
                self.txtDisplayName.invalideFieldWithError(strError: "Please enter valid name")
            }
            return false
        }
        guard "\(self.addChildParameters["dob"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
                self.txtDateOfBirth.invalideFieldWithError(strError: "Please enter valid DOB")
            }
            return false
        }
        guard "\(self.addChildParameters["email"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
                self.txtEmail.invalideFieldWithError(strError: "Please enter valid email address.")
            }
            return false
        }
        if let emailText:String = self.addChildParameters["email"] as? String,!emailText.isValidEmail(){
            DispatchQueue.main.async {
                self.txtEmail.invalideFieldWithError(strError: "Please enter valid email address.")
            }
            return false
...
//        }
    ...
    //MARK: - API Methods
    func requestAPIForAddChildSignUp(){
        self.addChildParameters["gender"] = self.isBoySelected ? "male" : "female"
        self.addChildParameters["device_type"] = "ios"
        self.addChildParameters["device_id"] = "\(UIDevice.current.identifierForVendor!.uuidString)"
        if let deviceToken = UserDefaults.standard.object(forKey: "currentDeviceToken") as? String{
            self.addChildParameters["device_token"] = deviceToken
        }else{
            self.addChildParameters["device_token"] = "iOS"
        }
        let imageData = self.userProfileImage?.jpeg(.medium) ?? nil
        //self.userProfileImage!.sd_imageData(as: .PNG, compressionQuality: 0.3)
        APIRequestClient.shared.uploadImage(requestType: .POST, queryString:kParentAddChild , parameter: self.addChildParameters as [String:AnyObject], imageData:imageData, isHudeShow: true, success: { (responseSuccess) in
    ...
    @IBAction func buttonSignUpSelector(sender:UIButton){
        if self.isValidSignUp(){
            self.requestAPIForAddChildSignUp()
        }
    }
...
extension AddChildViewController:CountryPickerDelegate{
    func didselectCountryCodewith(country: CountryCode) {
        self.selectedCountryCode = country.dialCode
        self.addChildParameters["country_code"] = "\(country.dialCode)"
    }
}
...
extension AddChildViewController:UITextFieldDelegate{
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        /*
         self.addChildParameters["name"] = "Test"
         self.addChildParameters["email"] = "darshanp12@itpathsolutions.in"
         self.addChildParameters["password"] = "ips12345"
         self.addChildParameters["confirm_password"] = "ips12345"
         self.addChildParameters["country_code"] = "91"
         self.addChildParameters["phone"] = "878787877887"
         */
        
        if textField == self.txtDisplayName{
            self.txtDisplayName.validateField()
            self.addChildParameters["name"] = "\(typpedString)"
        }else if textField == self.txtEmail{
            self.txtEmail.validateField()
            self.addChildParameters["email"] = "\(typpedString)"
        }else if textField == self.txtPassword{
            self.txtPassword.validateField()
            self.addChildParameters["password"] = "\(typpedString)"
        }else if textField == self.txtConfirmPassword{
            self.txtConfirmPassword.validateField()
            self.addChildParameters["confirm_password"] = "\(typpedString)"
        }else{
            self.txtPhoneNumber.validateField()
            self.addChildParameters["phone"] = "\(typpedString)"
        }
        print(self.addChildParameters)
        return true
    }
    func textFieldShouldClear(_ textField: UITextField) -> Bool {
        if textField == self.txtDisplayName{
            self.addChildParameters["name"] = ""
        }else{
            self.addChildParameters["phone"] = ""
        }
        return true
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        if textField == self.txtPassword{
            if let text = self.txtPassword.text{
                self.addChildParameters["password"] = text
            }else{
                self.addChildParameters["password"] = ""
            }
        }else if textField == self.txtConfirmPassword{
            if let text = self.txtConfirmPassword.text{
                self.addChildParameters["confirm_password"] = text
            }else{
                self.addChildParameters["confirm_password"] = ""
            }
        }
    }
}
