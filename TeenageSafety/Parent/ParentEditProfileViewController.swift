//
//  ParentEditProfileViewController.swift
//  TeenageSafety
//
...
class ParentEditProfileViewController: UIViewController {
    ...
    var updateParentProfileParameters:[String:Any] = [:]
    ...
    func setUp(){
        ...
        self.updateParentProfileParameters["country_code"] = "1"
    ...
    func configureCurrentUser(){
        if let objParent:Parent = Parent.getParentFromUserDefault(){
            ...
            DispatchQueue.main.async {
                self.txtDisplayName.text = objParent.parentName
                self.updateParentProfileParameters["name"] = objParent.parentName
                self.txtEmail.text = objParent.parentEmail
                self.updateParentProfileParameters["email"] = objParent.parentEmail
                self.buttonCountryCode.setTitle("\(objParent.parentCountryCode)", for: .normal)
                self.updateParentProfileParameters["country_code"] = objParent.parentCountryCode
                self.txtPhoneNumber.text = "\(objParent.parentPhone)"
                self.updateParentProfileParameters["phone"] = objParent.parentPhone
            }
        }
    }
    ...
    func configureFormDatePicker(){
        ...
        let doneButton = UIBarButtonItem(title: Vocabulary.getWordFromKey(key:"Done"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(ParentEditProfileViewController.doneFormDatePicker))
        ...
        title.sizeToFit()
        let cancelButton = UIBarButtonItem(title:Vocabulary.getWordFromKey(key:"Cancel"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(ParentEditProfileViewController.cancelFormDatePicker))
    ...
    @objc func doneFormDatePicker(){
        ...
        self.updateParentProfileParameters["dob"] = "\(self.fromDatePicker.date.yyyyMMdd)"
    ...
    func isValidSignUp()->Bool{
        ...
        guard "\(self.updateParentProfileParameters["name"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
        ...
        /*
        guard "\(self.updateParentProfileParameters["dob"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
                self.txtDateOfBirth.invalideFieldWithError(strError: "Please enter valid DOB")
            }
            return false
        }*/
        guard "\(self.updateParentProfileParameters["email"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
                self.txtEmail.invalideFieldWithError(strError: "Please enter valid email address.")
            }
            return false
        }
        if let emailText:String = self.updateParentProfileParameters["email"] as? String,!emailText.isValidEmail(){
            DispatchQueue.main.async {
                self.txtEmail.invalideFieldWithError(strError: "Please enter valid email address.")
            }
            return false
        ...
        /*
        guard "\(self.updateParentProfileParameters["password"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
                self.txtPassword.invalideFieldWithError(strError: "Please enter valid password")
            }
            return false
        }
        guard "\(self.updateParentProfileParameters["confirm_password"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
                self.txtConfirmPassword.invalideFieldWithError(strError: "Please enter valid confirm password")
            }
            return false
        }
        guard "\(self.updateParentProfileParameters["confirm_password"] ?? "")" == "\(self.updateParentProfileParameters["password"] ?? "")"  else {
            DispatchQueue.main.async {
                self.txtConfirmPassword.invalideFieldWithError(strError: "Confirm password not match to password")
            }
            return false
        }*/
        guard "\(self.updateParentProfileParameters["phone"] ?? "")".count > 0 else {
            DispatchQueue.main.async {
...
//        self.updateParentProfileParameters["gender"] = self.isBoySelected ? "male" : "female"
        self.updateParentProfileParameters["device_type"] = "ios"
        self.updateParentProfileParameters["device_id"] = "\(UIDevice.current.identifierForVendor!.uuidString)"
        if let deviceToken = UserDefaults.standard.object(forKey: "currentDeviceToken") as? String{
            self.updateParentProfileParameters["device_token"] = deviceToken
        }else{
            self.updateParentProfileParameters["device_token"] = "iOS"
        }
        let imageData = self.userProfileImage?.jpeg(.medium) ?? nil
        //self.userProfileImage!.sd_imageData(as: .PNG, compressionQuality: 0.3)
        APIRequestClient.shared.uploadImage(requestType: .POST, queryString:kParentUpdateProfile , parameter: self.updateParentProfileParameters as [String:AnyObject], imageData: imageData, isHudeShow: true, success: { (responseSuccess) in
...
extension ParentEditProfileViewController:CountryPickerDelegate{
    func didselectCountryCodewith(country: CountryCode) {
        self.selectedCountryCode = country.dialCode
        self.updateParentProfileParameters["country_code"] = "\(country.dialCode)"
    }
}

extension ParentEditProfileViewController:UIImagePickerControllerDelegate,UINavigationControllerDelegate{
...
extension ParentEditProfileViewController:UITextFieldDelegate{
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        /*
         self.updateParentProfileParameters["name"] = "Test"
         self.updateParentProfileParameters["email"] = "darshanp12@itpathsolutions.in"
         self.updateParentProfileParameters["password"] = "ips12345"
         self.updateParentProfileParameters["confirm_password"] = "ips12345"
         self.updateParentProfileParameters["country_code"] = "91"
         self.updateParentProfileParameters["phone"] = "878787877887"
         */
        
        if textField == self.txtDisplayName{
            self.txtDisplayName.validateField()
            self.updateParentProfileParameters["name"] = "\(typpedString)"
        }else if textField == self.txtEmail{
            self.txtEmail.validateField()
            self.updateParentProfileParameters["email"] = "\(typpedString)"
        }else if textField == self.txtPassword{
            self.txtPassword.validateField()
            self.updateParentProfileParameters["password"] = "\(typpedString)"
        }else if textField == self.txtConfirmPassword{
            self.txtConfirmPassword.validateField()
            self.updateParentProfileParameters["confirm_password"] = "\(typpedString)"
        }else{
            self.txtPhoneNumber.validateField()
            self.updateParentProfileParameters["phone"] = "\(typpedString)"
        }
        return true
    }
    func textFieldShouldClear(_ textField: UITextField) -> Bool {
        if textField == self.txtDisplayName{
            self.updateParentProfileParameters["name"] = ""
        }else{
            self.updateParentProfileParameters["phone"] = ""
        }
        return true
    }
