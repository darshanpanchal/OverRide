...
public class OBD2_BLE: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    var obdCommands:[String: String]! = [
        ...
        "fuel_level":"012F",
        "fuel_pressure":"010A",
        "engine_fuelrate":"015E",
        "engine_coolanttemp":"0105",
        "engine_oiltemp":"015C",
        "engine_load":"0104",
    ...
    public func getRPM() {
        if obd2 == nil || dataCharacteristic == nil {
            ...
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["RPM"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getEngineRuntime() {
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["engine_runtime"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    
    public func getfueltype(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["fuel_type"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getDistanceMIlon(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["distance_MILon"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getIntakemanifold(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["intake_manifold"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getAirinataketemp(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["air_intaketemp"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getFuellevel(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["fuel_level"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getFuelpressure(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands[" fuel_pressure"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    
    public func getEnginefuelrate(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["engine_fuelrate"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getEnginecooltemo(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["engine_coolanttemp"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getEngineoiltemp(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["engine_oiltemp"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getEngineload(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["engine_load"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    
    public func getBarometricpressure(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["barometric_pressure"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
    public func getAbsoluteload(){
        if obd2 == nil || dataCharacteristic == nil {
            print("Device is not connected to an OBD-II scanner.")
        } else {
            obd2!.writeValue(Data(bytes: Array("\(obdCommands["absolute_load"]!)\r".utf8)), for: dataCharacteristic!, type: .withResponse)
        }
    }
